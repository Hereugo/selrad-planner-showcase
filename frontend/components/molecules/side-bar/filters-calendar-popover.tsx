...
import { addDays, startOfISOWeek, startOfMonth, endOfMonth, addMonths, startOfYear, endOfYear, startOfToday } from "date-fns";
...
const CalendarFilterPopover: FC<CalendarFilterPopoverProps> = ({ children }) => {
    const defaultSelected = calendarOptions[3];
    const { calendarRange, setCalendarRange } = useFiltersContext();
    const [month, setMonth] = useState(calendarRange?.from || defaultSelected.from);
