...
export const useUpdatePlan = (initialPlan: Plan) => {
    ...
    const [managers, setManagers] = useState(initialPlan.managers.map((manager) => manager.id));
    const [worklist, setWorklist] = useState(initialPlan.worklist.map((work) => work.pk));
    ...
    const handleUpdatePlan = () => {
        if (!assignedDate || !client || !managers.length || !worklist.length || !shipmentCost || !boxCount) {
            toast({
